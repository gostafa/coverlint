...
func TestNewPolicyRejectsNonFiniteMinimums(t *testing.T) {
	...
	for _, minimum := range []float64{math.NaN(), math.Inf(1), math.Inf(-1), 0, -1, 100.1} {
		...
		t.Run("", func(t *testing.T) {
			...
			_, err := NewPolicy([]Rule{{Pattern: "**", Min: minimum}}, nil)
			if err == nil {
...
func TestEvaluateDeduplicatesProfileBlocksByPosition(t *testing.T) {
	...
	policy := mustPolicy(t)
	report := policy.Evaluate([]Package{{
		ImportPath: "github.com/acme/project/internal/orders",
		Dir:        "/repo/internal/orders",
		Files:      []string{"/repo/internal/orders/orders.go"},
		FirstFile:  "/repo/internal/orders/orders.go",
	}}, []Block{
		{File: "/repo/internal/orders/orders.go", Position: "10.1,12.2", Statements: 2, Covered: false},
		{File: "/repo/internal/orders/orders.go", Position: "10.1,12.2", Statements: 2, Covered: true},
		{File: "github.com/acme/project/internal/orders/orders.go", Position: "10.1,12.2", Statements: 2, Covered: false},
	})
...
func TestEvaluateDoesNotMapUnknownChildFileToAncestorPackage(t *testing.T) {
	...
	report := policy.Evaluate([]Package{{
		...
		{File: "/repo/internal/orders/subpkg/sub.go", Position: "1.1,2.2", Statements: 1, Covered: true},
	})
...
func mustPolicy(t *testing.T) Policy {
	...
	policy, err := NewPolicy([]Rule{{Pattern: "**", Min: 80}}, nil)
	if err != nil {
